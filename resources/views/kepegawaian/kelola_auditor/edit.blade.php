...
<p>Perbarui data auditor BSPJI Palembang.</p>
...
<form action="{{ route('kepegawaian.auditor.update', $auditor->id_auditor) }}" method="POST">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-4">
            ...
            class="form-control"
            value="{{ $auditor->nip ?? '' }}"
            placeholder="{{ (!empty($auditor) && $auditor->posisi == 'Subkon') ? 'Masukkan NIP (Opsional untuk Subkon)' : 'Masukkan NIP' }}">
        ...
        <div class="col-md-6 mb-4">
            ...
            <select name="status" class="form-select" required>
                ...
                <option value="Aktif" {{ (!empty($auditor) && $auditor->status == 'Aktif') ? 'selected' : '' }}>Aktif</option>
                <option value="Nonaktif" {{ (!empty($auditor) && $auditor->status == 'Nonaktif') ? 'selected' : '' }}>Nonaktif</option>
...
// Muat data kompetensi terisi secara default saat halaman dimuat
...
btnResetForm.addEventListener('click', function() {
    // Kembalikan ke state awal database (pada production diisi dengan data binding laravel)
    selectedKompetensi = [];
    renderKompetensiList();
    ruangLingkupSection.classList.add('d-none');
    ruangLingkupChecklist.innerHTML = '';
