...
<body>
    ...
    <div class="content">
        ...
        <div class="main">
            <!-- Header Title -->
            <div class="mb-4">
                <h2 class="fw-bold text-dark">Edit Riwayat Penugasan</h2>
                <p class="text-secondary mb-0">Silakan perbarui form di bawah ini untuk mengubah data riwayat penugasan auditor.</p>
            </div>
            ...
            <div class="form-box">
                <form action="{{ route('kepegawaian.riwayatauditor.update', $riwayat->id_riwayat) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <!-- Audit Perusahaan -->
                        <div class="col-md-6 mb-4">
                            <label class="form-label">Audit Perusahaan</label>
                            <select name="id_audit" class="form-select @error('id_audit') is-invalid @enderror" required>
                                @foreach($audits as $audit)
                                    <option value="{{ $audit->id_audit }}" {{ (old('id_audit', $riwayat->id_audit) == $audit->id_audit) ? 'selected' : '' }}>
                        ...
                        <!-- Jadwal Audit -->
                        <div class="col-md-6 mb-4">
                            <label class="form-label">Jadwal Audit</label>
                            <select name="id_jadwal" class="form-select @error('id_jadwal') is-invalid @enderror" required>
                                @foreach($jadwals as $jadwal)
                                    <option value="{{ $jadwal->id_jadwal }}" {{ (old('id_jadwal', $riwayat->id_jadwal) == $jadwal->id_jadwal) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <select name="status_penugasan" class="form-select @error('status_penugasan') is-invalid @enderror" required>
                                ...
                                <option value="Dibatalkan" {{ (old('status_penugasan', $riwayat->status_penugasan) == 'Dibatalkan') ? 'selected' : '' }}>Dibatalkan</option>
                            </select>
